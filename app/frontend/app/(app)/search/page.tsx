...
function SearchPage() {
  ...
  const filters: Params = React.useMemo(() => {
    ...
    return {
      ...
      granted: sp.get("granted") === "true" ? true : undefined,
  ...
  const [grantedFacet, setGrantedFacet] = React.useState<CountBucket[]>([]);
...
function buildChips(filters: Params, setFilter: (p: Partial<Params>) => void, niceMode: NiceMode): Chip[] {
  ...
  if (filters.granted) {
    chips.push({
      key: "granted",
      label: <>Granted</>,
      onRemove: () => setFilter({ granted: undefined }),
    });
  }
  if (filters.designated_country) {
    chips.push({
      key: "designated-country",
      label: <>Covers {filters.designated_country}</>,
...
function buildScopeLabel(filters: Params): string {
  ...
  if (filters.granted) parts.push("Granted");
  if (filters.designated_country) {
    parts.push(`Covers ${filters.designated_country}`);
  }
