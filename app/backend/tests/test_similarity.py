...
def test_composite_low_risk() -> None:
    ...
    c = composite_score(phonetic=0.2, visual=0.3, semantic=0.0, class_o=1.0, vienna_o=0.0)
    # mark_score = 0.35*0.2 + 0.15*0.3 = 0.115
    # mark_strength 0.30 sits at the ramp floor → goods_factor 0 → goods drop out
    # composite = 0.115
...
def test_composite_low_risk_when_only_class_overlaps() -> None:
    ...
    Under the v1.4 5-axis weights the composite is now 0.327 (the
    sub-0.5 mark signal also dampens the goods axis), comfortably
    Low risk — no longer the ~0.50 boundary case it was under the old
    weights. Class-only overlap with sub-0.5 mark signals → Low."""
    c = composite_score(phonetic=0.486, visual=0.423, semantic=0.0, class_o=1.0, vienna_o=0.0)
    # mark_score = 0.35*0.486 + 0.15*0.423 = 0.234
    # mark_strength 0.486 → goods_factor (0.486-0.30)/0.40 = 0.465
    # goods = 0.20*0.465 = 0.093
    # composite = 0.234 + 0.093 = 0.327
    assert c.verdict == "Low risk", (
...
def test_composite_per_matter_weights_override() -> None:
    ...
    pharma = {"phonetic": 0.70, "visual": 0.10, "semantic": 0.0, "class": 0.15, "vienna": 0.05}
    c = composite_score(phonetic=1.0, visual=0.0, semantic=0.0, class_o=1.0, vienna_o=0.0, weights=pharma)
    # composite = 0.70*1.0 + 0.10*0 + 0.0*0.0 + 0.15*1.0 + 0 = 0.85
...
def test_per_matter_weights_cannot_override_conjunction_guards() -> None:
    ...
    sketchy = {"phonetic": 1.0, "visual": 0.0, "semantic": 0.0, "class": 0.0, "vienna": 0.0}
    c = composite_score(phonetic=0.3, visual=0.0, semantic=0.0, class_o=0.0, vienna_o=0.0, weights=sketchy)
...
def test_typographic_visual_cannot_satisfy_conjunction_guard_alone() -> None:
    ...
    # mark_score = 0.35*0.476 + 0.15*0.559 = 0.250
    # mark_strength = phonetic-only 0.476 (typographic visual excluded),
    #   NOT 0.559 — typographic visual does not lift mark_strength
    # goods_factor = (0.476-0.30)/0.40 = 0.44 → goods = 0.20*0.44 = 0.088
    # composite = 0.250 + 0.088 = 0.338 → Low risk
    assert c.verdict == "Low risk", (
...
def test_composite_figurative_phash_visual_alone_is_low_risk() -> None:
    ...
    class guard passes — but the weighted composite lands 0.343 (recalibrated
    from 0.438 under the v1.4 weights), below the 0.50 floor, so the engine
    verdicts 'Low risk'.
    ...
    # mark_strength 0.95 (phash) → goods_factor 1.0 → goods = 0.20
    # composite = 0.1425 + 0.20 = 0.343 < 0.50 → Low risk
    assert c.verdict == "Low risk"
...
def test_phash_visual_does_satisfy_conjunction_guard() -> None:
    ...
    # mark_score = 0.35*0.30 + 0.15*0.95 = 0.2475
    # goods = 0.20*1.0 + 0.15*0.5 = 0.275
    # mark_strength = 0.95 (phash, despite phonetic 0.30) → goods_factor = 1.0
    # composite = 0.2475 + 0.275 = 0.522 ≥ 0.50, mark_strength ≥ 0.50 → Possible.
    assert c.verdict == "Possible conflict"
...
def test_goods_dampener_does_not_break_real_conflicts() -> None:
    ...
    must NOT demote a real conflict to Low. Under the v1.4 phonetic-
    protective reweight this near-identical pair dropped from 0.808 to
    0.669 (Likely → Possible): the goods dampener still passes full goods
    (mark_strength 0.945 ≥ 0.70), the pair still flags as a conflict, just
    at Possible severity rather than Likely. The test's point holds — the
    dampener does NOT silently bury a genuine near-twin."""
    c = composite_score(
        phonetic=0.945,
        visual=0.921,
        semantic=0.0,
    ...
    # mark_score = 0.35*0.945 + 0.15*0.921 = 0.469
    # composite = 0.469 + 0.20 = 0.669
    assert c.composite >= 0.50
    assert c.verdict == "Possible conflict"


def test_goods_dampener_pharma_shared_dominant_word_slips_to_low_after_reweight() -> None:
    """LIPITOR EXTRA vs LIPITAR PLUS — shared dominant pharma brand,
    descriptive suffixes vary. Under the v1.4 phonetic-protective reweight
    this borderline sound-only pair (phonetic 0.557) dropped from 0.521 to
    0.425, below the 0.50 floor → now 'Low risk'. This is the documented
    recall slip for borderline sound-only pairs — verdict-threshold
    recalibration is explicitly out of scope for this track (a later track
    if recall regresses), so the test records the new behaviour."""
    c = composite_score(
        phonetic=0.557,
        visual=0.675,
        semantic=0.0,
    ...
    # mark_strength = 0.557, goods_factor = (0.557-0.30)/0.40 = 0.643
    # mark_score = 0.35*0.557 + 0.15*0.675 = 0.296
