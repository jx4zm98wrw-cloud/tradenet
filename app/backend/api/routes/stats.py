...
async def overview(session: AsyncSession = Depends(get_session)) -> StatsOverview:
    ...
    g_inflight = (
        await session.execute(
            select(func.count())
            .select_from(Gazette)
            .where(Gazette.status.in_([GazetteStatus.uploaded, GazetteStatus.processing]))
        )
    ).scalar_one()
...
NICE_LABELS: dict[str, str] = {
    "01": "Chemicals",
    "02": "Paints",
    "03": "Cosmetics & cleaning",
    "04": "Fuels",
    "05": "Pharmaceuticals",
    "06": "Metal goods",
    "07": "Machines",
    "08": "Hand tools",
    "09": "Software & electronics",
