...
async def today_digest(
    ...
    last = await session.execute(select(Gazette.uploaded_at).order_by(desc(Gazette.uploaded_at)).limit(1))
...
class FindingOut(BaseModel):
    ...
    watchName: str
    reason: str
    matchedClasses: list[str] = []  # classes the watchlist queried for — UI uses to highlight


REASONS = [
    "Phonetic + class overlap",
    "Levenshtein 2 / class match",
    "Visual + class overlap",
    "Suffix match",
    "Identical root",
]
...
async def _findings_for_watchlist(
    ...
    from .watchlists import WatchQuery, _query_where
...
async def findings(
    ...
    seen_marks: set = set()
    for w in watchlists:
        watch_classes = (w.query or {}).get("nice_class") or []
        rows = await _findings_for_watchlist(session, w, per_list=3)
        for m, wl in rows:
            if m.id in seen_marks:
                continue
            seen_marks.add(m.id)
            mark_classes = set(m.nice_classes or [])
            matched = [c for c in watch_classes if c in mark_classes]
            out.append(
                FindingOut(
...
class PipelineStatsOut(BaseModel):
    ...
    gazettesTotal: int
    latestGazetteName: str | None
    latestGazetteRows: int | None
    latestGazetteAt: datetime | None
...
async def pipeline_stats(session: AsyncSession = Depends(get_session)) -> PipelineStatsOut:
    ...
    latest = (
        await session.execute(select(Gazette).order_by(desc(Gazette.uploaded_at)).limit(1))
    ).scalar_one_or_none()
