...
class WatchlistUpdate(BaseModel):
    ...
    client: Optional[str] = None
    matter: Optional[str] = None
    query: Optional[WatchQuery] = None
    queryDesc: Optional[str] = None
...
async def list_watchlists(session: AsyncSession = Depends(get_session)) -> List[WatchlistOut]:
    rows = (await session.execute(
        select(Watchlist).order_by(desc(Watchlist.new_count), desc(Watchlist.updated_at))
    )).scalars().all()
    return [_to_out(w) for w in rows]


@router.post("", response_model=WatchlistOut, status_code=201)
async def create_watchlist(body: WatchlistCreate, session: AsyncSession = Depends(get_session)) -> WatchlistOut:
    w = Watchlist(
        name=body.name, client=body.client, matter=body.matter,
...
async def update_watchlist(id: uuid.UUID, body: WatchlistUpdate, session: AsyncSession = Depends(get_session)) -> WatchlistOut:
    w = await session.get(Watchlist, id)
    if w is None:
        raise HTTPException(404, "Watchlist not found")
    if body.name is not None: w.name = body.name
    if body.client is not None: w.client = body.client
    if body.matter is not None: w.matter = body.matter
    if body.query is not None:
        ...
        w.new_count = new
        w.last_run_at = datetime.now(timezone.utc)
    if body.queryDesc is not None: w.query_desc = body.queryDesc
    await session.commit()
    await session.refresh(w)
    return _to_out(w)
...
async def delete_watchlist(id: uuid.UUID, session: AsyncSession = Depends(get_session)):
    w = await session.get(Watchlist, id)
    if w is None:
        raise HTTPException(404, "Watchlist not found")
...
async def watchlist_findings(
    id: uuid.UUID, limit: int = 12, session: AsyncSession = Depends(get_session),
) -> List[TrademarkOut]:
    w = await session.get(Watchlist, id)
    if w is None:
        raise HTTPException(404, "Watchlist not found")
...
def _query_where(q: WatchQuery):
    where = build_trademark_where(
        ...
        applicant_type=q.applicant_type, ip_agency=q.ip_agency,
    )
...
def _to_out(w: Watchlist) -> WatchlistOut:
    return WatchlistOut(
        id=w.id, name=w.name, client=w.client, matter=w.matter,
        query=WatchQuery(**w.query),
        queryDesc=w.query_desc,
        totalCount=w.total_count, newCount=w.new_count,
        createdAt=w.created_at, updatedAt=w.updated_at, lastRunAt=w.last_run_at,
    )


def _summarize_query(q: WatchQuery) -> str:
    parts: List[str] = []
    if q.q: parts.append(f'"{q.q}" ({q.mode})')
    if q.nice_class: parts.append(f"Classes {','.join(q.nice_class)} ({q.nice_class_mode.upper()})")
    if q.country: parts.append(f"Country {q.country}")
    if q.applicant_type: parts.append(q.applicant_type)
    if q.record_type: parts.append(q.record_type)
