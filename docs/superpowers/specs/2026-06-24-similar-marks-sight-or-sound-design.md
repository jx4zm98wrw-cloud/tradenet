...
- **Figurative pair with a real visual match → still included.** Two no-name marks (`mark_name` ∅) with pHash-similar logos in the same class/period → returned (the visual axis carries `mark_strength` past the conjunction guard). Confirms dropping the applicant fallback doesn't kill genuine figurative matches.
